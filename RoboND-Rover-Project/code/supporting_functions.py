...
def create_output_images(Rover):
      ...
      if rock_world_pos[0].any():
            ...
            for idx in range(len(Rover.samples_pos[0])):
                  ...
                  if np.min(rock_sample_dists) < 3:
                        ...
                        if (test_rock_x,test_rock_y) not in Rover.found_sample_pos:
                              Rover.found_sample_pos.append((test_rock_x,test_rock_y))
                        print(*Rover.found_sample_pos, sep='\n')
      ...
      sampleDist = "-"
      if Rover.sample_dists is not None: sampleDist = min(Rover.sample_dists)

      samplePixels = "-"
      if Rover.rock_pixels is not None: samplePixels = (Rover.rock_pixels)
      ...
      cv2.putText(map_add,"  Collected: "+str(Rover.samples_collected), (0, 85), 
                  cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
      #cv2.putText(map_add, "  Distance: " + str(sampleDist), (0, 100),
                  #cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
      #cv2.putText(map_add, "  Pix: " + str(samplePixels), (0, 115),
                  #cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
      # Convert map and vision image to base64 strings for sending to server
                  #cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
      #cv2.putText(map_add, "  vel0_count: " + str(Rover.vel0_count), (0, 115), cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
